...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = Path.home() / 'odoo-keyboard-layouts.config'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('odoo-keyboard-layouts.config', json.dumps(data))
    ...
    def _is_scanner(self):
        ...
        file_path = Path.home() / 'odoo-keyboard-is-scanner.config'
        if file_path.exists():
            data = json.loads(file_path.read_text())
    ...
    def _save_is_scanner(self, data):
        ...
        helpers.write_file('odoo-keyboard-is-scanner.config', json.dumps(data))
